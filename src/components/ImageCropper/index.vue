...
import effectRipple from './utils/effectRipple.js' // dom点击的波纹效果
export default {
  props: {
  ...
  methods: {
    ...
    upload() {
      ...
      this.loading = 1
      this.setStep(3)
      // 因为until里封装的axios做的有统一处理，而该请求url非当前项目后太url，所以里面没有code判断，会返回失败
      // 所以还是直接用axios吧
      // request({
      //   url,
      //   method: 'post',
      ...
      // })
      //   .then(resData => {
      //     console.log('avatar上传成功',resData);
      //     this.loading = 2
      //     this.$emit('crop-upload-success', resData.data)
      //   })
      //   .catch(err => {
      //     console.log('avatar上传失败',err);
      //     if (this.value) {
      //       this.loading = 3
      //       this.hasError = true
